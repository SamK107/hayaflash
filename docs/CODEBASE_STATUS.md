...
| Bandeau d'invitation à installer (Android + iOS) — `static/js/hf-install.js` : capture unique de `beforeinstallprompt` (3 copies dupliquées supprimées), `hfInstallInvite('buyer')` après commande réussie ou « M'alerter », `hfInstallInvite('seller')` sur les pages vendeur ; jamais si déjà installée, « Plus tard » = 7 jours de silence, iPhone = instructions « Partager → Sur l'écran d'accueil » ; bouton « Installer » permanent conservé | ✅ Fait (23/09) — à vérifier sur Android (HTTPS/ngrok) et iPhone |
...
> — 183 attendus avec les tests ajoutés le 23/09 (calendrier, boutique, manifests). (Le 20/09 : 169 tests au total.)
