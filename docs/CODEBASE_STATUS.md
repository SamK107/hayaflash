...
| Bandeau d'invitation à installer (Android + iOS) — `static/js/hf-install.js` : capture unique de `beforeinstallprompt` (3 copies dupliquées supprimées), `hfInstallInvite('buyer')` à la 1re visite d'une page acheteur (après 8 s, 1 fois / 24 h) puis après commande réussie ou « M'alerter » ; si le navigateur ne propose pas l'installation native, le bandeau affiche la marche à suivre manuelle (menu ⋮ / icône ⊕ / Partager sur iPhone) au lieu de rester invisible, `hfInstallInvite('seller')` sur les pages vendeur ; jamais si déjà installée, « Plus tard » = 7 jours de silence, iPhone = instructions « Partager → Sur l'écran d'accueil » ; bouton « Installer » permanent conservé | ✅ Fait (23/09) — à vérifier sur Android (HTTPS/ngrok) et iPhone |
| Invitation à installer — déclencheurs + navigateurs (24/09) | ✅ Fait — **acheteur** : uniquement après commande (`/f/`, `/order/`) ou « M'alerter » (invitation spontanée à 8 s supprimée) ; **vendeur** : uniquement après création/clonage d'une vente (flag de session `core.context_processors.request_pwa_install_invite`, consommé par `base.html` seulement — pas par un fragment HTMX), plus sur chaque page vendeur. Au plus 2 invitations par appareil et par contexte. `hfPwaInstall()` → dialogue natif (1 clic de confirmation imposé par le navigateur, incontournable) → `appinstalled` → message « HayaFlash est installée ✓ ». Marche à suivre par navigateur : iPhone, Firefox, Samsung Internet, Edge, Safari macOS, **navigateur intégré WhatsApp/Facebook/Instagram** (bouton « Ouvrir dans Chrome » via `intent://` sur Android). Marqueur `hf_installed` effacé si le navigateur repropose l'installation (app désinstallée). **Manifest choisi selon l'URL** (`core.context_processors.pwa_install`, préfixes `/f/`, `/s/`, `/ventes`, `/order/`) au lieu d'une surcharge par template : le futur dashboard « Découvrir » doit être placé sous `/ventes/` (ou un préfixe ajouté à `BUYER_PWA_PATH_PREFIXES`) pour recevoir le manifest acheteur. Test mobile HTTPS local : `runserver_https` (mkcert), voir `docs/DEV_HTTPS_MOBILE.md`. Tests : `flash_sales/tests.py::SellerInstallInviteTest`, `analytics/tests.py`. |
| Incohérences acheteur : ventes terminées affichées « EN DIRECT · termine dans 00:00:00 » dans `/ventes/`, bouton Commander actif sur `/ventes/<slug>/`, vente fermée par le vendeur affichée en direct sur `/f/` | ✅ Corrigé (24/09) — règle unique par l'heure `live_now_q()`/`upcoming_q()` (`flash_sales/services/ordering.py`) appliquée au calendrier, à `/s/`, à l'API publique, au sitemap et au dashboard vendeur ; état de `/f/` aligné sur la règle de commande ; `/ventes/<slug>/` → 301 `/f/<slug>/`. Tests `analytics/tests.py::PublicVisibilityByTimeTests`. Reste côté vendeur : la liste `/seller/flash-sales/` classe encore par statut brut. |
| Refonte `/ventes/` en liste compacte (En direct / Bientôt < 1 h / Aujourd'hui / Demain / dates), mise à jour automatique, catégorie de vente | ✅ Fait (24/09) — `flash_sales.public_views._calendar_groups` + `partials/_calendar_list.html` (rafraîchi par HTMX toutes les 45 s et dès qu'un compte à rebours franchit une limite) ; couleurs : en direct `#FF4D2E` + point pulsé, bientôt ambre, plus tard indigo (état toujours écrit en toutes lettres) ; styles locaux (build Tailwind figé). `core.choices.SaleCategory` : `SellerProfile.category` (profil) + `FlashSale.category` (vide = catégorie de la boutique, `effective_category`). |
| Page d'une vente terminée (`/f/<slug>/`, lien WhatsApp/QR) | ✅ Refaite (24/09) — plus de dates/prix/stock : preuve sociale (articles vendus en X min, clients servis, personnes en attente — chiffres nuls masqués) + carte « prochaine vente » du même vendeur (en direct ou programmée) + lien vers `/ventes/`. `_ended_sale_context` dans `analytics/services/public_pages.py`. « Tout vendu en X min » écarté : le stock est porté par le produit catalogue, partagé entre ventes (phase 9). |
| Icônes PWA acheteur nettes (écran de démarrage Android flou) | ✅ Fait (24/09) — régénérées jusqu'à 1024 px depuis le tracé vectoriel (`scripts/generate_pwa_icons.py`), même design (option « A nette » : fond orange, éclair blanc, éclair maskable 44 %). Android 12+ affiche l'icône de démarrage à ~240 dp (700-850 px) : le 512 px était agrandi. Écarté : carré arrondi sur l'écran de démarrage (Android utilise le même fichier pour l'icône d'accueil → « icône dans l'icône »), animation / nom en bas (non contrôlables par une PWA). Vendeur : même traitement (fond bleu nuit, éclair en dégradé du logo, 1024 px) — même script. |
...
> — 185 attendus avec les tests ajoutés le 23/09 (calendrier, boutique, manifests). (Le 20/09 : 169 tests au total.)
