...
6. **Limites de ventes/mois** (`PLAN_MONTHLY_SALES_LIMIT`) : FREE = 3, MEDIUM = 10, PRO = illimité. **Montants** : FCFA entiers partout, jamais de centimes — champs monétaires en `DecimalField(decimal_places=0)`, serializers sans décimales, montant envoyé tel quel à Orange Money (2000 = 2 000 FCFA) ; ne jamais multiplier ou diviser par 100.
...
9. **Healthcheck** (`config/api_urls.py:health` : DB + cache + battement Celery `core.celery_heartbeat`, check `celery` informatif sauf `HEALTH_CELERY_REQUIRED=true`) doit rester `@permission_classes([AllowAny])` et accessible à la fois sous `/health/` (racine, alias dans `config/urls.py`, ciblé par Dockerfile/compose/nginx) et `/api/v1/health/` — ne pas réintroduire `IsAuthenticated` dessus, ça casse tous les healthchecks infra silencieusement (403, pas d'erreur visible sans creuser).
...
12. **Vendoring front (14/09)** : Tailwind/HTMX/Alpine/Lucide/polices Inter+Poppins ne sont plus chargés depuis un CDN — build/téléchargement figé dans `static/vendor/` et `static/fonts/`, référencés via `{% static %}` dans `base.html`/`home.html`. Pour reconstruire le CSS Tailwind après un changement de classes dans les templates : voir `docs/FRONTEND_VENDORING.md`. Une `Content-Security-Policy` est active en **Report-Only** (`CSP_REPORT_ONLY = True`, policy dans `config/settings/_csp.py`). Depuis le sprint gouvernance B (26/09), `script-src 'self' 'unsafe-eval'` **sans `'unsafe-inline'`** : **aucun `<script>` inline ni attribut `on*=` dans les templates** (garde-fou `core/tests_csp.py`). Le JS va dans `static/js/` (`hf-base.js` = commun + actions déclaratives `data-hf-confirm`/`data-hf-copy`/`data-hf-show`…, `hf-public.js`, `hf-seller.js`, `hf-charts.js`), les données serveur passent par `data-*` ou `|json_script`. Les directives Alpine (`x-data`, `@click`) restent autorisées (`'unsafe-eval'`). Mode bloquant = étape 6 de `docs/RUNBOOK_JOUR_J.md`.
